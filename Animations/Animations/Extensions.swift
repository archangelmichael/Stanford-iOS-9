...
extension CGRect {
    ...
    var upLeft : CGPoint { return CGPoint(x: minX, y: minY) }
    var upRight : CGPoint { return CGPoint(x: maxX, y: minY) }
    var bottomLeft : CGPoint { return CGPoint(x: minX, y: maxY) }
    var bottomRight : CGPoint { return CGPoint(x: maxX, y: maxY) }
    
    init(center: CGPoint, size: CGSize) {
        let upLeft = CGPoint(x: center.x - size.width/2, y: center.y - size.height/2)
        self.init(origin: upLeft, size: size)
    }
}
